...
WINDOW_HEIGHT = 1000
DETECTOR = 'Eiger'
pv_Prefix = xrd_config.DP_PV_SECTOR + 'eiger'+ xrd_config.DP_PV_SUFFIX
...
class EigerFrame(wx.Frame):
	...
	def __init__(self, position=(400,500), parent=None, ClosePrompt=False, SaveRestore=False):
		wx.Frame.__init__(self, parent, title = 'DP Eiger Startup', pos = position, size = (WINDOW_WIDTH, WINDOW_HEIGHT), style=wx.DEFAULT_FRAME_STYLE ^ wx.RESIZE_BORDER)
		...
		self.helpDocs.Append(101, '&Eiger', '')					# Add entry
		...
		self.processes = {	'IOC':{	'pid': -999,
						'running': False,
						'search': 'eigerDetectorApp st.cmd',
						'file': ['/local/DPbin/Scripts/start_ioc', 'eiger'],
						},
					'MEDM':{'pid': -999,
						'running': False,
						'search': 'eigerDetector.adl',
						'file': '/local/DPbin/Scripts/start_medm_eiger',
						},
					...
					'caQtDM':{'pid': -999,
						'running': False,
						'search': 'caQtDM -macro P='+ pv_Prefix + ':, R=cam1: eigerDetector.ui',
						'file': ['/local/DPbin/Scripts/start_caQtDM_eiger',pv_Prefix,]
						},
					'IMAGEJ':{'pid': -999,
						'running': False,
		...
		# Title:									
		self.title = wx.StaticText(self.background, label="Eiger")
	...
	def helpDocs_101_Event(self, event):
		tempCommand = ['firefox', 'https://www1.aps.anl.gov/Detectors/Area-Detectors#Eiger']
