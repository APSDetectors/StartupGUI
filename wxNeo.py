...
WINDOW_HEIGHT = 1000
DETECTOR = 'Neo'
pv_Prefix = xrd_config.DP_PV_SECTOR + 'andor3'+ xrd_config.DP_PV_SUFFIX
...
class NeoFrame(wx.Frame):
	...
	def __init__(self, position=(400,500), parent=None, ClosePrompt=False, SaveRestore=False):
		wx.Frame.__init__(self, parent, title = 'DP Andor Neo Startup', pos = position, size = (WINDOW_WIDTH, WINDOW_HEIGHT), style=wx.DEFAULT_FRAME_STYLE ^ wx.RESIZE_BORDER)
		...
		self.helpDocs.Append(101, '&Neo', '')					# Add entry
		...
		self.processes = {	'IOC':{	'pid': -999,
					...
					'MEDM':{'pid': -999,
						'running': False,
						'search': 'Andor3.adl',
						'file': '/local/DPbin/Scripts/start_medm_andor3',
						},
					'IMAGEJ':{'pid': -999,
						'running': False,
		...
		# Title:									
		self.title = wx.StaticText(self.background, label="Andor Neo")
	...
	def helpDocs_101_Event(self, event):
		tempCommand = ['firefox', 'https://www1.aps.anl.gov/Divisions/XSD-Groups/Detectors/Scintillator-and-Visible-Light-Detectors#Andor']
